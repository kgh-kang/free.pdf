...
## 향후 과업 (낮은 우선순위, 사내 배포에는 영향 없음)
- **pdf.js worker self-host + SRI** — 현재 cdnjs CDN 직접 의존(`app.js:2`). cdnjs 침해 시 PDF 외부 송신 표면. self-host + SRI hash 갱신 워크플로 필요. "완전 로컬 처리" USP 강화 차원
- **다중 PDF 파일별 저장 (JSZip 도입)** — 현재 `editSourceNames.length>=2` 시 강제 `merged.pdf` 단일 출력(`app.js:1665`). 토글로 zip 분리 저장 추가 시 청구서/계약서 다건 도장 작업 1회 처리 가능
